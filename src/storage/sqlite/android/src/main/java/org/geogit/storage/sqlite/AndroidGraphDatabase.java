...
/**
 * Implementation of {@link GraphDatabase} based on Android SQLite api.
 * 
...
public class AndroidGraphDatabase extends SQLiteGraphDatabase<SQLiteDatabase> {
    ...
    protected SQLiteDatabase connect(File geogitDir) {
        File file = new File(geogitDir, "graph.db");
    ...
    protected void init(SQLiteDatabase cx) {
        ...
        sql = format("CREATE INDEX IF NOT EXISTS %s_nid_index ON %s(nid)", MAPPINGS, MAPPINGS);
        cx.execSQL(log(sql, LOG));
    ...
    protected String mapping(String node, SQLiteDatabase cx) {
        ...
        Cursor c = cx.query(MAPPINGS, array("nid"), "alias = ?", array(node), null, null, null);
        try {
            if (c.moveToNext()) {
    ...
    protected String property(String node, String key, SQLiteDatabase cx) {
        ...
        Cursor c = cx.query(PROPS, array("val"), "nid = ? AND key = ?", array(node, key), null,
                null, null);
        try {
    ...
    protected Iterable<String> outgoing(String node, SQLiteDatabase cx) {
        ...
        Cursor c = cx.query(EDGES, array("dst"), "src = ?", array(node), null, null, null);
    ...
    protected Iterable<String> incoming(String node, SQLiteDatabase cx) {
        ...
        Cursor c = cx.query(EDGES, array("src"), "dst = ?", array(node), null, null, null);
        return new StringCursorIterable(c);
    }
